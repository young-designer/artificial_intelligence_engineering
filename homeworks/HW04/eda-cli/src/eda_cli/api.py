...
# ---------- /quality-flags-from-csv: новый эндпоинт для HW03 ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    - has_constant_columns: есть ли колонки с одним уникальным значением
    - has_suspicious_id_duplicates: есть ли подозрительные дубликаты в ID-колонках
    
    Использует функции summarize_dataset, missing_table, compute_quality_flags из EDA-ядра.
    """
    ...
    # Используем EDA-ядро из core.py
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags_all = compute_quality_flags(summary, missing_df)

    # Извлекаем только булевы флаги для ответа
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
