...
### Добавлен новый HTTP-эндпоинт для HW03

**Эндпоинт:** `POST /quality-flags-from-csv`

**Назначение:** Получение полного набора флагов качества данных из CSV-файла, включая новые эвристики из HW03.

---

## Измененные файлы

### 3. `src/eda_cli/api.py`

**Добавлено:**

#### Новая модель ответа:
```python
# ДОБАВЛЕНО: Новая модель ответа для эндпоинта quality-flags-from-csv
...
#### Новый эндпоинт:
```python
# ДОБАВЛЕНО: Новый эндпоинт для получения полного набора флагов качества из CSV
# Реализует требования HW03 - использует новые эвристики качества данных
@app.post(
    "/quality-flags-from-csv",
    response_model=QualityFlagsResponse,
    tags=["quality"],
    summary="Получение полного набора флагов качества по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
```

**Функциональность:**
- Принимает CSV-файл через multipart/form-data
- Использует функции EDA-ядра: `summarize_dataset()`, `missing_table()`, `compute_quality_flags()`
- Возвращает только булевы флаги качества в JSON-формате
- Включает новые эвристики из HW03: `has_constant_columns`, `has_suspicious_id_duplicates`
...
**Отличия от существующих эндпоинтов:**
- `/health` - системный health-check
- `/quality` - принимает агрегированные признаки, возвращает полный QualityResponse
- `/quality-from-csv` - принимает CSV, возвращает полный QualityResponse с оценкой и сообщением
- `/quality-flags-from-csv` - **НОВЫЙ** - принимает CSV, возвращает только флаги качества
...
### Новый эндпоинт реализует требования HW03:
1. ✅ **Не дублирует** существующие эндпоинты `/health`, `/quality`, `/quality-from-csv`
2. ✅ **Использует доработки из HW03** - новые эвристики качества данных
3. ✅ **Возвращает полный набор флагов качества**, включая новые флаги:
   - `has_constant_columns` - обнаружение константных колонок
   - `has_suspicious_id_duplicates` - обнаружение дубликатов в ID-колонках

### Интеграция с EDA-ядром:
- Использует `summarize_dataset()` для анализа структуры данных
- Использует `missing_table()` для анализа пропусков
- Использует `compute_quality_flags()` с новыми эвристиками из HW03

### API-документация:
- Автоматически доступна по адресу `http://localhost:8000/docs`
- Включает описание нового эндпоинта и модели ответа
- Поддерживает интерактивное тестирование через Swagger UI